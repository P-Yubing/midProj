...
/**
 * 자카르타 프로젝트의 fileUpload모듈을 이용한 파일업로드 예제
 * @author PC404-01
 *
 */
@WebServlet("/controller/upload.do")
public class UploadController extends HttpServlet{
	
	//메모리 임계크기(이 크기가 넘어가면 레파지토리 위치에 임시파일로 저장됨)
	private static final int MEMORY_THRESHOLD = 1024 * 1024 * 3;
	//파일 1개당 최대 크기
	private static final long MAX_FILE_SIZE = 1024 * 1024 * 40; //한개 40까지
	//요청파일 최대 크기
	private static final long MAX_REQUEST_SIZE = 1024 * 1024 * 50; //여러개 파일을 선택할 수 있음 50까지
				...
				if(fileItems != null && fileItems.size() > 0) {
					for(FileItem item : fileItems) {
						if(!item.isFormField()) { //파일인 경우...
							...
							String filePath = "/ClassPic/" + fileName;
